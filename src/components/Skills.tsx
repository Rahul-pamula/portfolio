    ...
    <section id="skills" className="py-24 bg-[#0a0508] relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border text-left transition-all duration-300 ${
                    isActive 
                      ? "bg-white/5 border-brand-start/30 shadow-glow-red" 
                      : "bg-[#050305] border-white/5 hover:bg-white/5 hover:border-white/10"
                  }`}
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-colors ${
                    isActive ? "bg-[#1f0d14] border-brand-start/50" : "bg-black/50 border-transparent"
                  }`}>
                    {category.icon}
                  </div>
                  <div>
          ...
          <div className="lg:col-span-8 glass-panel rounded-3xl p-8 md:p-12 min-h-[300px] flex items-center">
            {skillCategories.map((category) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-8 pb-4 border-b border-white/10">
                    {category.icon}
                    <h3 className="text-2xl font-bold font-display text-white">{category.title} Stack</h3>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {category.skills.map((skill, idx) => (
                      <motion.div
                        ...
                        className="px-4 py-2.5 rounded-xl bg-black/40 border border-white/10 text-gray-300 font-semibold hover:border-brand-start/50 hover:bg-brand-start/10 hover:shadow-glow-red hover:text-white transition-all cursor-default backdrop-blur-sm"
                      >
                        {skill}
                      </motion.div>
                    ))}
